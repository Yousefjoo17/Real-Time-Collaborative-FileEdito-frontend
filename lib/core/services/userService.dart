...
class UserService {
  ...
  Future<List<UserModel>> getAllUsers() async {
    ...
    try {
      Response response = await dio
          .get("http://localhost:8080/Real-Time-Collaborative-Editing/users");
  ...
  Future<void> createUser(UserModel user) async {
    try {
      await dio.post(
          "http://localhost:8080/Real-Time-Collaborative-Editing/users",
          data: user.toJson());
    } catch (e) {
      throw Exception("Error: $e");
    }
  ...
  int getCurrUserID(List<UserModel> users, UserModel newUser) {
    for (var user in users) {
      if (user.username == newUser.username &&
          user.password == newUser.password) {
        return user.userID!;
